...
<body>
	...
	<%   String  te=(String)session.getAttribute("branches");
	     ...
	     for(int i=0;i<branches.length;i++){%>
	    	 <a href="main_file?path=&branch=<%=branches[i]%>" style="color: #27A9E3; font-size: 16px;"><%=branches[i]%>分支</a>
	    	 &nbsp;&nbsp;&nbsp;&nbsp;
	   <%  }
	...
	<% 	 String[] temp=dir.split("/");
	     if(!dir.equals("")){%>
	    	 <a href="main_file?path=&branch=<%=branch%>" style="color: #000000; font-size: 16px;text-decoration: none;font-weight:bold">分支<%=branch+":"+"_" %>根目录cseiii/</a>
	   <%   for(int k=0;k<temp.length;k++){%>
	         ...
	         %>
	    	 <a href="main_file?path=<%=path0%>&branch=<%=branch%>" style="color: #000000; font-size: 16px;text-decoration: none;font-weight:bold"><%=temp[k]%>/</a>
	 <% }
	    }else{%>
	    	<a href="main_file?path=&branch=<%=branch%>" style="color: #000000; font-size: 16px;text-decoration: none;font-weight:bold">分支<%=branch+":"+"_" %>根目录cseiii/</a>
	 <% }%>
	     ...
	     <% if(!dir.contains(".")){
	        ...
	        %><img src="images/files.jpg">
	    	 <a href="main_file?path=<%=dir+path[i]+"/"%>&branch=<%=branch%>" style="color: #000000; font-size: 16px;text-decoration: none"><%=path[i]%></a>
	    	 <br>
	    	 <% 
	          }else{
	    	 ...
	    	 <a href="main_file?path=<%=dir+al+"/"%>&branch=<%=branch%>" style="color: #000000; font-size: 16px;text-decoration: none"><%=path[i]%></a>
	    	 <br> 
	          <%}
	    	 %>
	     <%}
