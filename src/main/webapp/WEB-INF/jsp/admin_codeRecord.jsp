     ...
     <%   String hash=(String)session.getAttribute("hash");
          if(hash.equals("0")){%>
        	 <%   String  te=(String)session.getAttribute("codebranches");
	    		 ...
	    		 <a href="admin_codeRecord?hash=0&branch=<%=branches[i]%>&gid=<%=gid %>" style="color: #27A9E3; font-size: 16px;"><%=branches[i]%>分支</a>
	    	 &nbsp;&nbsp;&nbsp;&nbsp;
	   <%  	}
	...
	%>
        ...
        <% 	String temp=(String)session.getAttribute("coderecord");
          	...
          	for(int i=0;i<record.length;i++){ 
        	   ...
        	   <a href="admin_codeRecord?hash=<%=a[3]%>&branch=<%=branch%>&gid=<%=gid %>" style="color: #000000; font-size: 16px;font-weight:bold">提交说明:<%=a[0] %></a>
                <br>
